...
export const Canvas: React.FC = () => {
	...
	const initialDPRRef = useRef(false);
	...
	useEffect(() => {
		...
		initialDPRRef.current = true;
	}, [setupCanvasDPR]);

	useEffect(() => {
		if (canvasRef.current && initialDPRRef.current) {
			setupCanvasDPR(canvasRef.current);
		}
	}, [activeLayer?.id, setupCanvasDPR, pointer]);

	// useEffect(() => {
	// 	if (isHistoryActive) {
	// 		Object.values(canvasesRef.current).forEach(canvas => {
	// 			setupCanvasesDPR(canvas);
	// 			console.log('2');
	// 		});
	// 	}
	// }, [setupCanvasesDPR, pointer, isHistoryActive]);
