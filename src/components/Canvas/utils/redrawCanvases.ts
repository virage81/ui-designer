...
export const redrawCanvases = (
	canvas: Record<string, HTMLCanvasElement>,
	objects: Drawable[] = [],
	pointer: number,
) => {
	Object.values(canvas).forEach((canvasElement) => {
		if (!canvasElement || !(canvasElement instanceof HTMLCanvasElement)) {
			console.error('Invalid canvas element:', canvasElement);
			return;
		}
		...
		if (!ctx) {
			console.error('Failed to get 2D context for canvas:', canvasElement);
			return;
		}
		...
		objects.forEach((obj) => {
			if (obj.pointer >= 0 && pointer >= obj.pointer && canvasElement.id === obj.layerId && !obj.removed) {
				switch (obj.type) {
					case 'brush': {
